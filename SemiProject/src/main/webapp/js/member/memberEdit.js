		 ...
		 $.ajax({
			 ...
			 success:function(json){
				 ...
				 if(json.isExists) {
					 ...
					 $("span#emailCheckResult").html($("input#user_email").val() + " 은 이미 사용중 이므로 다른 이메일을 입력하세요").css({"color":"red"}); 
					 $("input#user_email").val(""); 
				 }
				 else {
					 // 입력한 email 이 존재하지 않는 경우라면 
					 $("span#emailCheckResult").html($("input#user_email").val() + " 은 사용가능 합니다").css({"color":"white"});  
				 }
