	         ...
	         return;  // 종료
	          ...
	          if(!"admin".equals(userid) ) {
	        	...
	        	// admin(관리자)으로 로그인 한 경우 
	              ...
	              // System.out.println("odrcodeoptno" +odrcodeoptno);
	              // System.out.println(odrcodeoptno);
	              ...
	              try {
	                  n = pdao.updateDeliverStart(odrcodeoptno);
	              ...
	              if(n == odrcodeArr.length) {
	                 ...
	                 for(String key : odrcodeMapKeysets) {
	                   ...
	                   //String api_secret = "발급받은 본인의 API Secret";  // 발급받은 본인 API Secret
	                    ...
	                    paraMap.put("to", mvo.getUser_phone()); // 수신번호
	                    paraMap.put("from", "01031417056"); // 발신번호
	                    paraMap.put("type", "SMS"); // Message type ( SMS(단문), LMS(장문), MMS, ATA )
	                    paraMap.put("text", "블리자드 엔터테인먼트에서 "+mvo.getUser_name()+"님 께서 주문하신 전표["+key+"]를 우체국택배로 배송했습니다."); // 문자내용    
