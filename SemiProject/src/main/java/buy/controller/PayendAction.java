...
public class PayendAction extends AbstractController {
		...
		if(super.checkLogin(request)) {
	        ...
	        MemberVO loginuser = (MemberVO) session.getAttribute("loginuser");
	         ...
	         if(loginuser.getUser_id().equals(user_id)) {
	        	...
	        	String productName = user_name+"님의 주문상품"; // :"새우깡"; 등등 으로 변경가능 받아와야함
	     		int productPrice = 500; 
	     		//Integer.parseInt(realpaymoney);
	     		...
	     		request.setAttribute("realpaymoney", realpaymoney); // db 업데이트용 
